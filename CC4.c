...
skim(opstr, testfunc, dropval, endval, heir, lval)
	...
	while (1) {
		...
		if (streq(line + lptr, opstr) == 2) {
			...
			const (dropval);
			postlabel(endlab);
			lval->indirect = lval->ptr_type = lval->is_const = lval->const_val =
					0;
...
plnge2b(heir, lval, lval2, oper, foper)
	...
	if (oper == zsub) {
		...
		if (lval->ptr_type == CINT && lval2->ptr_type == CINT) {
			...
			const1(1);
			asr(); /*  div by 2  */
		} else if (lval->ptr_type == FLOAT && lval2->ptr_type == FLOAT) {
			swap();
			const1(6);
			div(); /* div by 6 */
...
heir1a(lval)
	...
	if (cmatch('?')) {
		...
		if (lval2.val_type == FLOAT && lval->val_type != FLOAT) {
			fpcall("_float##");
			lval->val_type = FLOAT;
			postlabel(endlab);
		} else if (lval2.val_type != FLOAT && lval->val_type == FLOAT) {
...
heira(lval)
	...
	if (match("++")) {
		...
		/* [AC]  this is the cause of the variableName(&ar[2])problem */
		/*  removed line below to avoid the null lval->symbol-type error need to check  */
		lval->ptr_type = lval->symbol->type;
		/* toconsole(); */
		outstr(";DEBUG--------> heira - debug 1\n");
		lval->val_type = CINT;
		if (lval->indirect)
			return 0;
		/* global & non-array */
		outstr(";DEBUG--------> heira - debug 2\n");
		address(lval->symbol);
