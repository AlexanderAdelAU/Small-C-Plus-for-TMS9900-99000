...
skim(opstr, testfunc, dropval, endval, heir, lval)
	char *opstr;int (*testfunc)(), dropval, endval, (*heir)();
	LVALUE *lval; {
...
plnge2a(heir, lval, lval2, oper, uoper, foper)
	int (*heir)();
	LVALUE *lval, *lval2;int (*oper)(), (*uoper)(), (*foper)(); {
	char *before, *start;
	...
	if (lval->is_const) {
		...
		/* one or both operands not constant - TMS 9900 modified for unsigned */
		if (lval->is_unsigned || lval2->is_unsigned) {
			(*uoper)();
			lval->binop = uoper;
		} else {
			(*oper)();
...
plnge2b(heir, lval, lval2, oper, foper)
	int (*heir)();
	LVALUE *lval, *lval2;int (*oper)(), (*foper)(); {
...
heir1(lval)
	...
	if (cmatch('=')) {
		...
		oper = zmod;
	else if (match(Rshfteq))
		oper = asr;
	else if (match(Lshfteq))
		oper = asl;
	else
		return k;
...
heir234(lval, heir, opch, oper)
	LVALUE *lval;int (*heir)();char opch;int (*oper)(); {
	int k;
...
heir5(lval)
	...
	k = plnge1(heir6, lval);
	blanks();
	if ((streq(line + lptr, Eqeq) == 0) && (streq(line + lptr, Noteq) == 0))
		return k;
	if (k)
		rvalue(lval);
	while (1) {
...
heir6(lval)
	...
	k = plnge1(heir7, lval);
	blanks();
	if (ch() != '<' && ch() != '>' && (streq(line + lptr, Lteq) == 0)
			&& (streq(line + lptr, Gteq) == 0))
		return k;
	if (streq(line + lptr, Rshft))
		return k;
	if (streq(line + lptr, Lshft))
		return k;
	if (k)
		rvalue(lval);
	while (1) {
...
heir7(lval)
	...
	k = plnge1(heir8, lval);
	blanks();
	if ((streq(line + lptr, Rshft) == 0) && (streq(line + lptr, Lshft) == 0))
		return k;
	if (streq(line + lptr, Rshfteq))
		return k;
	if (streq(line + lptr, Lshfteq))
		return k;
	if (k)
		rvalue(lval);
	while (1) {
	...
	if (ch() == '[' || ch() == '(' || ch() == '.'
		...
		while (1) {
			if (cmatch('[')) {
				...
				if (symname(sname) == 0
						|| (ptr = findmemb(lval->tagsym, sname)) == 0) {
				...
				if (ptr->ident == ARRAY
						|| (ptr->type == STRUCT && ptr->ident == VARIABLE)) {
					/* array or struct */
					lval->ptr_type = ptr->type;
					lval->val_type = CINT;
					k = 0;
