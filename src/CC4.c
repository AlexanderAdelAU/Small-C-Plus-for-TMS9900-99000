...
char Rshft[]	= ">>" ;
char Rshfteq[]	= ">>=" ;
char Subscript[]	= "can't subscript" ;
 ...
 */
plnge2a(heir, lval, lval2, oper, uoper, foper)
int (*heir)() ;
LVALUE *lval, *lval2 ;
int (*oper)(), (*uoper)(), (*foper)();
{
	char *before, *start ;
	...
	if ( foper == 0 ) intcheck(lval, lval2) ;
	if ( widen(lval, lval2) ) {
		(*foper)();
		/* result of comparison is int */
		if( foper != fmul && foper != fdiv )
			lval->val_type = CINT;
		return;
	}
	...
	if ( lval->is_const ) {
		...
		lval->const_val = calc(lval->const_val, lval2->const_val, oper) ;
		clearstage(before, 0) ;
	}
	else {
 ...
 */
plnge2b(heir, lval, lval2, oper, foper)
int (*heir)() ;
LVALUE *lval, *lval2 ;
int (*oper)(), (*foper)() ;
{
...
LVALUE *lval ;
{
	...
	else if ( match("+=") ) { oper = zadd; foper = fadd; }
	else if ( match("-=") ) { oper = zsub; foper = fsub; }
	else if ( match("*=") ) { oper = mult; foper = fmul; }
	else if ( match("/=") ) { oper = div; foper = fdiv; }
	else if ( match("%=") ) oper = zmod;
	else if ( match(Rshfteq) ) oper = asr;
	else if ( match(Lshfteq) ) oper = asl;
	else return k;
	...
	if ( k == 0 ) {
		...
		if ( oper==zadd || oper==zsub )
			plnge2b(heir1, lval, &lval2, oper, foper) ;
		else
			plnge2a(heir1, lval, &lval2, oper, oper, foper) ;
		smartstore(&lval3) ;
		return 0 ;
	}
}
...
LVALUE *lval ;
{
	...
	k = plnge1(heir6, lval) ;
	blanks() ;
	if((streq(line+lptr,Eqeq)==0) &&
		(streq(line + lptr,Noteq)==0))return k;
	if ( k ) rvalue(lval) ;
	while(1) {
		LVALUE lval2 ;
...
LVALUE *lval ;
{
	...
	blanks() ;
	if ( ch() != '<' && ch() != '>' &&
		(streq(line+lptr,Lteq)==0) &&
		(streq(line+lptr,Gteq)==0) ) return k ;
	if ( streq(line+lptr,Rshft) ) return k ;
	if ( streq(line+lptr,Lshft) ) return k ;
	if ( k ) rvalue(lval) ;
	while(1) {
		LVALUE lval2 ;
...
LVALUE *lval ;
{
	...
	k = plnge1(heir8, lval) ;
	blanks();
	if((streq(line+lptr,Rshft)==0) &&
		(streq(line+lptr,Lshft)==0))return k;
	if ( streq(line+lptr, Rshfteq) ) return k ;
	if ( streq(line+lptr, Lshfteq) ) return k ;
	if ( k ) rvalue(lval) ;
	while(1) {
		LVALUE lval2 ;

		if ((streq(line+lptr,Rshft) == 2) &&
			(streq(line+lptr,Rshfteq) == 0) ) {
			inbyte();
			inbyte();
			plnge2a(heir8, lval, &lval2, asr, asr, 0) ;
		}
		else if ((streq(line+lptr,Lshft) == 2) &&
			(streq(line+lptr,Lshfteq) == 0) ) {
			inbyte();
			inbyte();
...
LVALUE *lval ;
{
	...
	while(1) {
		...
		if (cmatch('+')) {
			plnge2b(heir9, lval, &lval2, zadd, fadd) ;
		}
		else if (cmatch('-')) {
			plnge2b(heir9, lval, &lval2, zsub, fsub) ;
		}
		else return 0 ;
	}
}
...
LVALUE *lval ;
{
	...
	while(1) {
		...
		if (cmatch('*')) {
			plnge2a(heira, lval, &lval2, mult, mult, fmul);
		}
		else if (cmatch('/')) {
			plnge2a(heira, lval, &lval2, div, div, fdiv);
		}
		else if (cmatch('%')) {
...
LVALUE *lval ;
{
	...
	/* in either case, symbol cannot be stack relative */
	lval->storage = STATIK ;
	return lval->symbol ;
}
...
LVALUE *lval ;
{
	...
	if(match(Plusplus)) {
		...
		if (heira(lval)) rvalue(lval) ;
		if (lval->val_type == DOUBLE) callrts("_ifix##"); ;
		lneg() ;
		lval->const_val = !lval->const_val ;
		lval->val_type = CINT ;
		lval->stage_add = NULL ;
		...
		k = heirb(lval) ;
		if ( match(Plusplus) ) {
			poststep(k, lval, 1, inc, dec, inct, dect) ;
			return 0;
		}
		else if ( match(Minmin) ) {
			poststep(k, lval, -1, dec, inc, dect, inct ) ;
			return 0;
		}
		else return k;
	}
