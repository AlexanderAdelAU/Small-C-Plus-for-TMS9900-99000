...
poststep(k, lval, n, step, unstep, step2, unstep2)
	int k;
	LVALUE *lval;int n, (*step)(), (*unstep)(), (*step2)(), (*unstep2)(); {
	if (k == 0) {
		needlval();
	} else {
		if (lval->indirect)
 ...
 *
 * poststep() has already preserved the lvalue address on the runtime
 * stack when lval->indirect is non-zero.  Do NOT push the old pointer
 * value here: store() would then pop that value as though it were the
 * lvalue address and write through the pointer instead of updating the
 * pointer variable.
 *
 * Keep the post-increment expression value in R4 by stepping to the new
 * pointer, storing it, then stepping R4 back to the original value.
 * This path is used for structure and double pointers, where the step is
 * larger than the simple char/int pointer cases handled by poststep().
 */
nstep(lval, n)
	LVALUE *lval;int n; {
	addconst(n);
...
size_of(lval)
	...
	case CCHAR:
	case UCCHAR:
		/* A char is one C byte.  Local stack slots may still be two bytes. */
		sz = 1;
		break;
	case DOUBLE:
		sz = 6;
