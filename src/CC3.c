	...
	while ( ch() != ')' ) {
		...
		if ( ptr ) {
			...
			if (type == FLOAT) {
				...
				zpushC();		/* [AC] need special attention when passing chars */
				nargs += 2;
			} else {
				zpush();
				nargs += 2;
...
addglb(sname, id, typ, class, value, storage, more, itag)
...
int value, storage, more, itag;
{
	...
	addsym(glbptr, sname, id, typ, class, storage, more, itag) ;
...
int more, itag ;
{
	...
	cptr = locptr++ ;
	addsym(cptr, sname, id, typ, 0, STKLOC, more, itag) ;
...
int value, storage, more, itag ;
{
	...
	addsym(membptr, sname, id, typ, class, storage, more, itag) ;
 ...
 */
addsym(ptr, sname, id, typ, class, storage, more, itag)
SYMBOL *ptr ;
char *sname, id, typ, class ;
...
k =0;
	...
	if ( alpha(ch()) == 0 )
		return (*sname=0) ;
	k = 0 ;
	/* Ensure symbols of single length are given an extra character to overcome
	 * hashing errors in the assembler.
	 */
...
preprocess()
{
	...
	/* Added by Alex Cameron 10th April 2020 during the COVID-19 outbreak
	  to allow comments to be made using the forward slash */
...
addmac()
{
	...
	addglb(sname, MACRO, 0, 0, macptr, STATIK, 0, 0) ;
	while ( ch()==' ' || ch()=='\t' ) gch() ;
	while ( putmac(gch()) ) ;
