...
getmem(sym)
	...
	} else if (sym->ident != POINTER && sym->type == FLOAT) {
		address(sym->name);
		fpcall("_fload##");
	} else {
		ot("\tMOV @");
...
putmem(sym)
	SYMBOL *sym; {
	ol(";putmem(sym)");
	if (sym->ident != POINTER && sym->type == FLOAT) {
		address(sym->name);
		fpcall("_fstore##");
	} else {
		if (sym->ident != POINTER && sym->type == CCHAR) {
 ...
 * bt not needed now and can be removed at some stage as it is equivalent to zpush()
 */
zpushC() {
	ol(";zpushC()");
	ol("\tDECT SP");
...
peephole(ptr, output)
	char *ptr;int output; {
	char *tptr;
	return 0; /* disable until generation is finalised */
	while (*ptr) {
		if (streq(ptr, "\tLI R4,")) {
			tptr = ptr + 7;
			while (*tptr != '\n')
