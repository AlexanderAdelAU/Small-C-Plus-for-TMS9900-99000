...
char Target[] =
		"        TMS9900 Relocatable Version 18th May, 2021 - Alexander Cameron ";
char Banner[] =
		"* * *  Small-C/Plus Version 1.06a for the TMS 9900 - Cameron * * *";
char Author[] = "       Cain, Van Zandt, Hendrix, Yorston, Cameron";

/*
 *	1.06  Original port for Z80 1.06 version - A.Cameron
 *	1.06a Added unsigned modifier for char and int - - A.Cameron
 *
 */
...
/*
 * TSP call-graph sidecar output.
 *
 * Enabled by the -T compiler switch.  This is deliberately observational:
 * it does not alter the generated .A99 stream.
 */
FILE *tspgraph;
int tspgraphflag;

#ifdef SMALL_C
int minavail = 64000; /* minimum memory available */
#endif
...
main(argc, argv)
	...
	tspgraph = NULL; /* no TSP call-graph sidecar */
	...
	return (errcnt ? 1 : 0); /* then exit to system */
}
...
SYMBOL* get_dummy(idx)
	int idx; {
	return dummy_sym[idx];
}
...
dumplits(size, pr_label)
	...
	if (k & 1)
		even(); /* TMS 9900 addition to force even boundary *5 5*/
}
...
ask() /* fetch arguments */
{
	...
	mainflg = 1; /* If a source does not have main, e.g. library files then use -M to remove main code.*/
	i = 1;
	while (i < gargc) {
		ptr = gargv[i++];
		if (ptr[0] != '-')
...
openout() {
	...
	/*
	 * -T writes a separate call-graph file.  Keep this independent of
	 * assembler output so enabling TSP analysis cannot perturb .A99.
	 */
	if (tspgraphflag) {
		strcpy(cgname, Filename);
		strcat(cgname, ".TCG");
		if ((tspgraph = fopen(cgname, "w")) == NULL) {
			error("Can't open TSP call graph");
		} else {
			fputs("TSPCALLS 1\nMODULE ", tspgraph);
			fputs(Filename, tspgraph);
			fputs("\n", tspgraph);
		}
	}
	clear(); /* erase line */
...
/*
 * TSP call-graph sidecar helpers.
 *
 * Records are intentionally simple ASCII:
 *   DEF   function
 *   CALL  caller callee       normal stack CALL
 *   BL    caller callee       leaf/runtime BL
 *   ICALL caller              indirect CALL target unknown
 *   ASM   function            raw #asm occurs in this function
 *
 * Application function names are written exactly as outname() presents
 * them to R99.  This matters for one-character C names, which outname()
 * prefixes with "q_".  Runtime helper strings are recorded raw because
 * those generators use outstr(), not outname().
 */
tspgraph_symname(name)
	char *name;
{
	int i;
	...
	char *name;
{
	...
	fputs(kind, tspgraph);
	fputs(" ", tspgraph);
	tspgraph_symname(currfn->name);
	fputs(" ", tspgraph);
	tspgraph_symname(name);
	fputs("\n", tspgraph);
}

tspgraph_call_raw(kind, name)
	char *kind;
	char *name;
{
	if (!tspgraph || !currfn)
...
TAG_SYMBOL* defstruct(sname, storage, s_memb)
	char *sname;int storage;int s_memb; {
	...
	++nam[1];
	itag = ptr - tagtab;
	dummy_sym[NTYPE + 1 + itag] = addglb(nam, POINTER, STRUCT, 0, 0, STRUCT, 0,
			0, itag);
	...
	/*
	 * TMS9900 ABI: every structure/union occupies an even number of bytes.
	 * This keeps every element of an array of structures word aligned and
	 * makes tag->size the single authoritative stride used by sizeof() and
...
/*
 * Return the TMS9900 alignment required for a structure member.
 * Characters are byte aligned; all word-valued objects and aggregates are
 * word aligned.  Arrays inherit the alignment of their element type because
 * declglb() leaves the element type in "type" (arrays of pointers become CINT).
 */
member_align(ident, type)
	int ident, type; {
	if (ident == POINTER)
		return 2;
	switch (type) {
	...
	do {
		...
		if (mtag == 0) {
			...
			/* Note use of typ so unsigned can be determined */
				...
				addglb(sname, ident, type,
						(storage != EXTERNAL && ident != FUNCTION) ? ENT : EXT,
						(ident == FUNCTION) ? EXTERNAL : storage, more, itag);
			}
		} else if (s_memb) {
			/*
			 * TMS9900 ABI: align word-valued members before assigning their
			 * offsets.  Character members remain byte packed.
			 */
			if (member_align(ident, type) == 2 && (mtag->size & 1))
				mtag->size++;

			/* are adding structure member, mtag->size is now its aligned offset */
	...
	TAG_SYMBOL *otag; /* tag of struct for object being declared */
{
	...
	if (swactive || noloc)
		error("not allowed here (switch/goto)");
	if (declared < 0)
	...
	do {
		...
		if (cmatch('[')) {
			...
			/* Scale the true object size first, then pad only the stack slot. */
			cscale(type, otag, &size);
			if (size & 1)
				size++; /* TMS9900 stack allocation must remain even */
		} else if (ident == PTR_TO_PTR) {
			ident = POINTER;
			more = dummy_idx(typ, otag);
			type = CINT;
			...
			case STRUCT:
				size = otag->size;
				if (size & 1)
					size++; /* For TMS 9900 storage must be even bytes */
				break;
			default:
				size = 2;
			}
...
initials(sname, type, ident, dim, more, tag)
	...
	TAG_SYMBOL *tag; {
	int size;
	if (type == STRUCT) /* TMS990 Check added for non char types */
	...
	if (cmatch('=')) {
		...
		outdec(dim);
		nl();
		if (dim & 1) {/* for TMS9900 to force even *//*55*/
			even();
		}
...
/*
 * Emit exact byte padding inside an explicitly initialised structure.
 * BYTE 0 is used rather than EVEN because member padding is part of the
 * structure's layout and may occur in the middle of the object.
 */
str_pad(count)
	int count; {
	while (count-- > 0) {
...
str_init(tag)
	...
	used = 0;
	while (ptr < tag->end) {
		/* Reproduce any alignment gap recorded in the member offset. */
		if (ptr->offset.i > used) {
			str_pad(ptr->offset.i - used);
			used = ptr->offset.i;
		}

		/* Mirror the number of bytes init() actually emits for scalar members. */
		bytes = (ptr->type == CINT || ptr->ident == POINTER) ? 2 : 1;
		dim = 1;
	...
	/* Include final structure padding so one initialised object is tag->size bytes. */
	if (tag->size > used)
		str_pad(tag->size - used);
}
...
newfunc(nam, seen_close)
	...
	*iptr; /* temporary ptr for stepping along arg chain */
	...
	/* TSP call-graph definition record; no effect on .A99 output. */
	tspgraph_def(currfn->name);
